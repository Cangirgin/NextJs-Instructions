...
import { convertFormDataToJson, getYupErrors } from "@/helpers/functions/form-validation";
...
export const updateProductAction = async (prevState, formData) => {
    ...
    try {
        ...
        const resp = await fetch(`${config.apiBaseURL}/products2/${fields.id}`, {
            method: "put",
            body: JSON.stringify(fields),
            headers: {
                "Content-Type": "application/json",
...
export const deleteProductAction = async (id) => {
    try {
        ...
        const res = await fetch(`${config.apiBaseURL}/products2/${id}`, {
            method: "delete",
        });
